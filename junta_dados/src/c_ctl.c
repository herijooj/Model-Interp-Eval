...
int open_ctl(info_ctl* info_field, char* name){
    ...
    //nome do arquivo
    fscanf(ctl_file,"%*s %"STR(STR_SIZE)"s\n",buff);
    if (buff[0] == '^'){
    ...
    //pula uma linha
    fgets(buff,BUFF_SIZE,ctl_file);

    //undef
    fscanf(ctl_file,"%*s %f\n",&(info_field->undef));

    //xdef
    fscanf(ctl_file,"%*s %lu %*s %f %f\n", &(info_field->x.def), &(info_field->x.i), &(info_field->x.size));
    ...
    //zdef
    fscanf(ctl_file,"%*s %lu",&(info_field->zdef));
    fgets(buff,BUFF_SIZE,ctl_file);

    //tdef
    fscanf(ctl_file,"%*s %lu %*s",&(info_field->tdef));
    fgets(info_field->tdesc,STR_SIZE,ctl_file);

    //resto do arquivo
    fread(info_field->dump,1,BUFF_SIZE,ctl_file);
...
binary_data* aloca_bin(size_t x, size_t y, size_t t){
    ...
    if ( !(bin_data->data = malloc( x*y*t*sizeof(float) )) ){
        free(bin_data);
        return NULL;
    }
...
binary_data* open_bin(char* name, size_t x, size_t y, size_t t){
    ...
    //alocando matriz de dados
    bin_data->data = malloc(sizeof(float) * x * y * t);
    if(!bin_data->data){
        fclose(bin_file);
        free(bin_data);
        return NULL;
    ...
    fread(bin_data->data,sizeof(float),x*y*t,bin_file);
...
int write_bin(binary_data* bin_data){
    ...
    if ( fwrite(bin_data->data, sizeof(float), dims, bin_file) < (dims)){
...
float get_data_val(binary_data* ref, binary_data* src, int x, int y, int t){
    ...
    // verifica se os indices passados estão na matriz
    if(!contains(ref,x,y,t)) return ref->info.undef;
    ...
    if(!contains(src, x_src, y_src, t_src)) return ref->info.undef;
...
float cp_data_val(binary_data* dest, binary_data* src, int x, int y, int t){
    if(contains(dest,x,y,t)){
        return dest->data[get_pos(&(dest->info),x,y,t)] = get_data_val(dest,src,x,y,t);
    }
